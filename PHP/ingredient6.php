<?php
	...
	$sql = "SELECT * FROM $table AS t1 JOIN (SELECT CEIL(RAND()*(SELECT MAX(id) FROM $table)) AS id) AS t2 WHERE t1.id >= t2.id AND MATCH(ingredient1, ingredient2, ingredient3, ingredient4, ingredient5, ingredient6, ingredient7, ingredient8, ingredient9, ingredient10, ingredient11, ingredient12, ingredient13, ingredient14, ingredient15) AGAINST('$ingredient1', '$ingredient2', '$ingredient3', '$ingredient4', '$ingredient5', '$ingredient6') ORDER BY t1.id ASC LIMIT 1";
